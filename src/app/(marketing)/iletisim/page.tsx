...
import { Send, CheckCircle, FileUp, Sparkles, Mail, Phone, MapPin } from "lucide-react";
      ...
      <div className="max-w-4xl mx-auto px-6 space-y-12">
        ...
        <div className="text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold text-slate-900 mb-4 tracking-tight">
            Bizimle <span className="text-transparent bg-clip-text bg-gradient-to-r from-pink-600 to-cyan-600">İletişime Geçin</span>
          </h1>
          <p className="text-lg text-slate-600 max-w-xl mx-auto leading-relaxed">
            Projeleriniz, sorularınız veya iş birlikleriniz için formu doldurarak ekibimize doğrudan ulaşabilirsiniz.
          </p>
        </div>

        {/* İletişim Formu Kutusu */}
        <div className="bg-white/95 backdrop-blur-xl border border-pink-100/80 rounded-[2.5rem] p-8 md:p-12 shadow-2xl shadow-pink-500/5 relative overflow-hidden">
          ...
          <div className="flex items-center gap-3 mb-8 border-b border-slate-100 pb-4">
            ...
            <div>
              <h3 className="text-xl font-bold text-slate-900">Doğrudan İletişim Formu</h3>
              <p className="text-xs text-slate-500">Tüm talepleriniz merkezi yönetim sistemimize iletilir.</p>
            </div>
          </div>
          ...
          {submitted ? (
            <div className="bg-green-50 border border-green-100 rounded-3xl p-8 text-center space-y-3">
              ...
              <h4 className="text-xl font-bold text-green-900">Mesajınız İletildi!</h4>
              <p className="text-green-700 text-sm">Talebiniz başarıyla kaydedilmiştir. En kısa sürede geri dönüş sağlanacaktır.</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid gap-6 md:grid-cols-2">
                ...
                <div>
                  ...
                  <input required type="text" placeholder="Üniversite veya Kurum Adı" className="w-full rounded-2xl border border-slate-200 bg-white p-4 text-sm focus:ring-2 focus:ring-pink-500 outline-none" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase mb-2">Sınıf / Departman *</label>
                  <input required type="text" placeholder="Örn: Moleküler Biyoloji / Ar-Ge" className="w-full rounded-2xl border border-slate-200 bg-white p-4 text-sm focus:ring-2 focus:ring-pink-500 outline-none" />
                </div>
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase mb-2">Notunuz / Sorularınız *</label>
                <textarea rows={4} required placeholder="Mesajınızı veya talebinizi detaylı bir şekilde yazın..." className="w-full rounded-2xl border border-slate-200 bg-white p-4 text-sm focus:ring-2 focus:ring-pink-500 outline-none"></textarea>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="border-2 border-dashed border-slate-200 rounded-3xl p-6 text-center bg-slate-50 hover:border-pink-500 transition-all cursor-pointer">
                  ...
                  <p className="text-sm font-medium text-slate-600">İlgili belge veya dosyayı buraya yükleyin</p>
